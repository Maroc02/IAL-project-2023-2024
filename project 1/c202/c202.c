...
void Stack_Init( Stack *stack ) {
	if (stack == NULL) // Stack is a NULL pointer
		Stack_Error(SERR_INIT); // Error function
	else {
		stack->array = malloc(sizeof(char) * STACK_SIZE); // Dynamically allocate memory for the stack
		stack->topIndex = -1; // Stack is empty by default
	}
}
...
bool Stack_IsEmpty( const Stack *stack ) {
	return (stack->topIndex == -1); // Automatically returns true / false
}
...
bool Stack_IsFull( const Stack *stack ) {
	return (stack->topIndex == STACK_SIZE - 1); // Automatically returns true / false
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if (Stack_IsEmpty(stack)) // Stack is empty
		Stack_Error(SERR_TOP); // Error function
	else
		*dataPtr = stack->array[stack->topIndex]; // Get the top char from the stack
}
...
void Stack_Pop( Stack *stack ) {
	...
	stack->topIndex--; // Delete the top char
}
...
void Stack_Push( Stack *stack, char data ) {
	if (Stack_IsFull(stack)){ // Stack is full
		Stack_Error(SERR_PUSH); // Error function
	} else{
		stack->topIndex++; // Increase stack by one
		stack->array[stack->topIndex] = data; // Insert a new top char
	}
}
...
void Stack_Dispose( Stack *stack ) {
	stack->topIndex = -1; // Set stack back to empty state
	free(stack->array); // Free the dynamically allocated memory 
	stack->array = NULL; // Set the pointer to NULL
}
